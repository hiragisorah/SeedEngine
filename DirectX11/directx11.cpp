...
void DirectX11::initialize(void * const handle, const float2 & size)
{
...
const float2 & DirectX11::getSize(void) const
{
	return size_;
}

void DirectX11::clearRenderTarget(const std::vector<RenderTargetID>& render_target_id_s, const color & color)
{
...
void DirectX11::setShader(const int & shader_slot, const ShaderID & shader_id)
{
	...
	context_->VSSetShader(shader->vs_.Get(), nullptr, shader_slot);
	context_->GSSetShader(shader->gs_.Get(), nullptr, shader_slot);
	context_->HSSetShader(shader->hs_.Get(), nullptr, shader_slot);
	context_->DSSetShader(shader->ds_.Get(), nullptr, shader_slot);
	context_->PSSetShader(shader->ps_.Get(), nullptr, shader_slot);

	context_->VSSetConstantBuffers(shader_slot, 1, shader->cb_.GetAddressOf());
	context_->GSSetConstantBuffers(shader_slot, 1, shader->cb_.GetAddressOf());
	context_->HSSetConstantBuffers(shader_slot, 1, shader->cb_.GetAddressOf());
	context_->DSSetConstantBuffers(shader_slot, 1, shader->cb_.GetAddressOf());
	context_->PSSetConstantBuffers(shader_slot, 1, shader->cb_.GetAddressOf());
}
...
void DirectX11::setDiffuse(const color & diffuse)
{
	per_mesh_cb_.diffuse_ = diffuse;
}

void DirectX11::setColor(const color & color)
{
	per_camera_cb_.color_ = color;
}

void DirectX11::setWorld(const matrix & world)
{
	per_mesh_cb_.world_ = world;
}

void DirectX11::setView(const matrix & view)
{
	per_camera_cb_.view_ = view;
}

void DirectX11::setProjection(const matrix & projection)
{
...
void DirectX11::createRenderTarget(const RenderTargetID & render_target_id, const DescFormat & format, const float2 & size)
{
...
void DirectX11::createDepthStencil(const DepthStencilID & depth_stencil_id, const float2 & size)
{
...
void DirectX11::createViewPort(const ViewPortID & view_port_id, const float2 & size)
{
