...
#include "directx11Defs.hpp"
#include "directx11Enums.hpp"
...
public:
	void initialize(void * const handle, const float2 & size);
...
public:
	void clearRenderTarget(const std::vector<RenderTargetID> & render_target_id_s, const color & color);
...
public:
	void createRenderTarget(const RenderTargetID & render_target_id, const DescFormat & format, const float2 & size);
	void createDepthStencil(const DepthStencilID & depth_stencil_id, const float2 & size);
	void createViewPort(const ViewPortID & view_port_id, const float2 & size);
